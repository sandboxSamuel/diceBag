...
function bodyLoaded(doc) {
  ...
  document
    .getElementById("modifier-slider")
    .addEventListener("input", checkModifier, false);
}
...
function addDice(diceSize) {
  ...
  newDie.setAttribute(
    "id",
    "d" + diceSize + "-" + Math.floor(Math.random() * 1000 + 1)
  );
...
function checkModifier() {
  ...
  if (modifier != 0) {
    ...
    if (modifier > 0) queuedMod.innerHTML = "+" + modifier;
...
function clearOldValues() {
  ...
  if (oldModifier != null) oldModifier.remove();

  var oldTotal = document.getElementById("total-display");
  if (oldTotal != null) oldTotal.remove();
...
function clearQueue() {
  ...
  while (queued[0]) queued[0].remove();
...
function logRoll(dice, modifier, total) {
  ...
  for (diceCount = 0; diceCount < dice.length; diceCount++) {
    ...
    loggedDie.classList.add(
      "dice-button",
      "d" + getDiceSize(dice[diceCount]),
      "logged-dice"
    );
...
function playDiceSound() {
  if (
    document
      ...
      .classList.contains("glyphicon-volume-off")
  )
    return;
...
document.getElementById("rollD3").addEventListener("click", () => {
  const result = rollD3();
  console.log(result);
  displayResult("D3", result);
});
